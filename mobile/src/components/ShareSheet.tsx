...
import { View, Text, TouchableOpacity, Modal, Linking, Platform, Share, ActivityIndicator } from 'react-native';
...
export default function ShareSheet({ visible, onClose, document, userId, apiBase }: { visible: boolean; onClose: () => void; document: DocumentItem; userId: string; apiBase?: string; }) {
  ...
  useEffect(() => {
    ...
    async function fetchSigned() {
      ...
      try {
        ...
        const r = await fetch(`${base}/.netlify/functions/signed-urls?userId=${encodeURIComponent(userId)}&appId=${encodeURIComponent(idParam)}`);
        if (!ignore) {
          if (r.ok) {
    ...
    return () => { ignore = true; };
  }, [visible, document.appId, document.id, userId, apiBase]);

  async function copyLinks() {
    const text = [baseMessage, signedLinksText].filter(Boolean).join('\n');
    try {
  ...
  function shareWhatsApp() {
    const text = encodeURIComponent([baseMessage, signedLinksText].filter(Boolean).join('\n'));
  ...
  function shareEmail() {
    ...
    const body = encodeURIComponent([baseMessage, signedLinksText].filter(Boolean).join('\n'));
